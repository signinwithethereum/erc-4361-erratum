...
status: internal scaffolding (not submitted; informs PR on ethereum/EIPs branch review/4361)
---

# Proposed diffs for ERC-4361 remediation

Companion to [`inconsistencies.md`](./inconsistencies.md) and `action-plan.md` (maintainer-internal, not published). For each finding we give:

- **Replacement text**: minimal edit to `erc-4361.md`. Line numbers refer to the local copy of [`erc-4361-snapshot.md`](./erc-4361-snapshot.md) as of 2026-04-23.
...
- **Compat tag**: one of `Editorial`, `Clarification`, `Narrow`, `Widen`, `Judgment` (per action-plan §"Per-finding compatibility classification").

Ordering matches the Stage 2 commit order in `action-plan.md` (maintainer-internal, not published) §2.2 — low-controversy first — not the inconsistencies.md numbering.
...
**Tag:** Judgment → harmonize DOWN to SHOULD (per action-plan §"Judgment call: #3").
...
**Rationale:** `*pchar` permits the empty string, so `Request ID: \n` parses. The field description describes `request-id` as "a system-specific identifier"; a zero-length identifier is unlikely to be intended. However, the canonical test-vectors (`libs/test-vectors/vectors/grammar/valid_specification.json:request-id empty`) explicitly document the "present but empty" form as valid, with `requestId: ""` as the expected parsed result — the distinction between "missing" and "present but empty" is intentional. We therefore keep the grammar permissive and add producer-side guidance without breaking existing parsers.
...
**Revision note:** This commit was originally specified as an ABNF narrowing (`request-id = 1*pchar`). The conformance scan of `libs/test-vectors/` surfaced a positive vector requiring the empty form to parse (see `conformance-matrix.md` §3.1). The commit was softened to prose-only guidance to preserve the intentional "present but empty" semantic.
...
**Compatibility:** Narrows. `libs/test-vectors/vectors/parsing/parsing_positive.json` currently includes two positive vectors — `domain is RFC 3986 authority with userinfo` and `domain is RFC 3986 authority with userinfo and port` — which this commit would move out of conformance. These vectors are RFC-3986 grammar-completeness tests (domain = `test@127.0.0.1`), not real SIWE-usage samples. No production SIWE relying party is known to emit `userinfo@host` in a SIWE message; the phishing case depends on attacker-controlled messages.

### Coordinated release plan (unique to this commit)

This is the only surviving narrowing where the canonical `libs/test-vectors/` corpus must change in lockstep. The sequencing matters:

1. **Pre-submission.** Open a PR against `libs/test-vectors` that moves the two userinfo vectors from `parsing/parsing_positive.json` into `parsing/parsing_negative.json` (or a new `parsing_deprecated_positive.json` file, if we want to preserve the history). Do not merge yet. The test-vectors PR title should explicitly reference the upcoming ERC-4361 erratum, e.g. `test-vectors: reclassify userinfo@ domain vectors (ERC-4361 erratum)`.
2. **Pre-submission.** Open draft PRs against `libs/ts`, `libs/py`, `libs/rs`, and `libs/go` that update their parser grammar to reject `userinfo@` in `domain`. Each draft PR imports the updated test-vectors revision as a dependency.
3. **Submission day.** Open the EIPs PR. In its body, link to the four downstream draft PRs and the test-vectors PR as "concurrent release plan evidence." Editors and reviewers see a complete, coordinated rollout, not a spec change with a dangling implementation gap.
...
5. **Fallback.** If the EIPs editor asks to drop the narrowing, close the four downstream draft PRs and the test-vectors PR with a reference to the editor decision. No wasted work shipped.
...
Message Fields, line 114 (note: Commit 19's widening will replace the charset language; this commit only adds the presence/empty guidance — the two are sequenced as the "Notes for Stage 2" section describes):
...
| 17 | #6 | Narrow (+ corpus migration) | `erc-4361.md` (ABNF via Commit 15) + `libs/test-vectors` concurrent PR |
...
- Line numbers are snapshot-dependent. Before opening the PR, re-verify every `Line NN` citation against `EIPS/eip-4361.md` in `/home/jalil/1001/ethid/siwe/EIPs` at branch `review/4361` — renumbering from the editorial commits (1–4) will shift later lines.
- Commits 6, 14, 15 all edit line 112 (`domain` field description). They must be applied in that order and each must rebase cleanly onto its predecessor; the text grows with each edit.
- Commits 18 and 19 both edit line 114. Commit 18 lands first (prose-only, adds producer SHOULD-omit guidance), Commit 19 second (widens the charset). The resulting final text for line 76 is `statement = 1*( %x20-7E )` (Commit 19 alone — Commit 18 no longer touches the ABNF). The resulting final text for line 114 contains both the widening language and the presence/empty guidance.
- Commit 17 (#6 userinfo) is the only commit whose success depends on an out-of-tree PR landing in lockstep: the `libs/test-vectors` reclassification PR described in Commit 17's release plan. Track that PR alongside the EIPs branch through Stages 2–4.
- Commit 20 (reference implementation) does not depend on any specific numbering in `erc-4361.md`; it depends on the cumulative normative state. It is scheduled last so it reflects that cumulative state without rework.
